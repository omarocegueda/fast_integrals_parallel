...
def rectangle_sums_sequential(double[:,:,:] I, int[:] m, double[:,:,:] out):
    cdef:
        ...
        double integral
        double[:, :, :] T = np.zeros((2, n1, n2), dtype=np.float64)
    with nogil:
        cur_i = 1
        for i in range(n0):
            cur_i = 1 - cur_i
    ...
    with nogil, parallel():
        ...
        for i in range(s_padded, s_end):
            ...
            for j in range(n1):
                for k in range(n2):
                    # Start with last corner
                    T[cur_i][j][k] = I[i,j,k] # q=(0,0,0)
                    # Add signed rectangles
                    if i>s_padded: # Our "zero" is now the first slice we are processing in this thread
                        T[cur_i][j][k] += T[prev_i][j][k]# q=(1, 0, 0)
                        if j>0:
                            T[cur_i][j][k] -= T[prev_i][j-1][k]# q=(1, 1, 0)
                            if k>0:
                                T[cur_i][j][k] += T[prev_i][j-1][k-1]# q=(1, 1, 1)
                        if k>0:
                            T[cur_i][j][k] -= T[prev_i][j][k-1]# q=(1, 0, 1)
                    if j>0:
                        T[cur_i][j][k] += T[cur_i][j-1][k]# q=(0, 1, 0)
                        if k>0:
                            T[cur_i][j][k] -= T[cur_i][j-1][k-1]# q=(0, 1, 1)
                    if k>0:
                        T[cur_i][j][k] += T[cur_i][j][k-1]# q=(0, 0, 1)
                        
                    # Add displaced signed corners
                    if i>=s_padded + m0: # Our "zero" is now the first slice we are processing in this thread
                        T[cur_i][j][k] -= I[i-m0,j,k]# q=(1, 0, 0)
                        if j>=m1:
                            T[cur_i][j][k] += I[i-m0,j-m1,k]# q=(1, 1, 0)
                            if k>=m2:
                                T[cur_i][j][k] -= I[i-m0,j-m1,k-m2]# q=(1, 1, 1)
                        if k>=m2:
                            T[cur_i][j][k] += I[i-m0,j,k-m2]# q=(1, 0, 1)
                    if j>=m1:
                        T[cur_i][j][k] -= I[i,j-m1,k]# q=(0, 1, 0)
                        if k>=m2:
                            T[cur_i][j][k] += I[i,j-m1,k-m2]# q=(0, 1, 1)
                    if k>=m2:
                        T[cur_i][j][k] -= I[i,j,k-m2]# q=(0, 0, 1)
                        
                    
                    # Use integral of current rectangle
                    if (s_start <= i) and (i < s_end):
                        out[i,j,k] = T[cur_i][j][k]
        free_volume(T, 2, n1)                
